...
WHERE NAME IN (
    SELECT NAME FROM (
        SELECT NAME, RANK() OVER (ORDER BY PRICE DESC) AS RNK FROM PRODUCT
        GROUP BY NAME
    ) T
    WHERE RNK = 1
)
...
SELECT * FROM APP_SALE
UNION ALL
SELECT ..., NULL AS USER_ID FROM STORE_SALE
...
JOIN EMPLOYEE M FROM E.MANAGER_ID=M.ID
...
SELECT COUNT(T.TEACHER_ID) AS COUNT FROM TEACHER T
LEFT JOIN CLASS C ON T.TEACHER_ID = C.TEACHER_ID로
